...
```html
<iframe id="pm-research-compendium" src="https://pennmediated.github.io/research-compendium/" title="Research Compendium — Penn MEDIATED" scrolling="no" allow="clipboard-write" style="width:100%;height:14000px;border:0;display:block"></iframe><script>(function(){var f=document.getElementById('pm-research-compendium');if(!f)return;var t=null,loaded=false;if(location.hash){f.src='https://pennmediated.github.io/research-compendium/'+location.hash;}window.addEventListener('message',function(e){if(e.source!==f.contentWindow)return;var d=e.data||{},h=d.frameHeight||(d.type==='partners-page-resize'?d.height:0);if(h)f.style.height=h+'px';if(typeof d.frameScrollTo==='number'){t=d.frameScrollTo;go(loaded?'smooth':'auto');}});function go(b){if(t===null)return;window.scrollTo({top:f.getBoundingClientRect().top+window.pageYOffset+t-120,behavior:b});}window.addEventListener('load',function(){loaded=true;if(location.hash)setTimeout(function(){go('auto');},60);});window.addEventListener('hashchange',function(){f.contentWindow.postMessage({pmHash:location.hash},'*');});})();</script>
```
...
- Divi scrolls the WordPress page back to the top once it finishes loading whenever the address has a `#hash`. On a link opened in a new tab that can land after the jump to the entry, so the page "bounces" to the top. Both sides now guard against it: the snippet remembers the last scroll target and re-applies it just after the WordPress page's `load`, and the page itself repeats its scroll request a few times in the first three seconds after load (`holdInitialScroll()`), stopping as soon as the visitor scrolls, taps or types. The page-side half works with an older snippet too; the snippet half needs the snippet above pasted in.

...
Top to bottom the page is: title and intro with the newsletter panel beside it → the filter strip → the entry list. **The two theme charts ("Entries by theme", "Themes over time") are hidden for now** (September 2026): their `<section class="glance">` carries the `hidden` attribute. Delete that attribute to bring them back; the chart code still runs, so nothing else changes. The charts follow the filters, and selecting a bar or a heatmap cell sets the theme (and year) filter.
...
- Uppercase micro-labels ("Center newsletter", the column headers, the theme chips) are DM Sans 700 uppercase at `--fs-micro`.
...
- Each entry ends with a footer row: "Read paper ⟶" (category 2), "Copy entry link" and the "Full summary & details" toggle on the left, the theme tags on the right under the takeaway column (they drop below, left-aligned, on phones). Opening the entry puts the summary above that footer, so the same toggle — now "Show less" — stays at the bottom and closing it scrolls back to the top of the entry.
...
```html
<iframe id="pm-research-compendium" src="https://pennmediated.github.io/research-compendium/" title="Research Compendium — Penn MEDIATED" scrolling="no" allow="clipboard-write" style="width:100%;height:14000px;border:0;display:block"></iframe><script>(function(){var f=document.getElementById('pm-research-compendium');if(!f)return;var base='https://pennmediated.github.io/research-compendium/',h=location.hash,t=null,loaded=false,user=false,fu=false,hold=h?Infinity:0;if(h){f.src=base+h;try{history.replaceState(null,'',location.pathname+location.search);}catch(e){}}['wheel','touchstart','keydown','mousedown'].forEach(function(ev){window.addEventListener(ev,function(){user=true;},{passive:true,once:true});});window.addEventListener('message',function(e){if(e.source!==f.contentWindow)return;var d=e.data||{},hh=d.frameHeight||(d.type==='partners-page-resize'?d.height:0);if(hh)f.style.height=hh+'px';if(d.frameUserScroll)fu=true;if(typeof d.frameScrollTo==='number'){if(user&&Date.now()<hold)return;t=d.frameScrollTo;go(loaded?'smooth':'auto');}});function go(b){if(t===null)return;window.scrollTo({top:f.getBoundingClientRect().top+window.pageYOffset+t-120,behavior:b});}window.addEventListener('load',function(){loaded=true;if(h){hold=Date.now()+4000;setTimeout(function(){if(!user&&!fu)go('auto');try{history.replaceState(null,'',location.pathname+location.search+h);}catch(e){}},60);}});window.addEventListener('hashchange',function(){f.contentWindow.postMessage({pmHash:location.hash},'*');});})();</script>
```
...
### Entry links ("Copy entry link")

"Copy entry link" copies `https://mediated.upenn.edu/research-compendium/#<entry id>` (the address is `PAGE_URL` in the script). Opening it expands that entry and scrolls to it. A cross-origin iframe can't see the WordPress page's address, so the snippet above does three extra things, and all three matter:

- The `<iframe>` has a normal `src`, so the page shows even if the script never runs (the Divi Visual Builder doesn't run Code-module scripts in its preview). When the WordPress page's address has a `#hash`, the script reloads the frame with that hash appended, so it opens already knowing which entry to show. (No `loading="lazy"`: a lazy frame wouldn't load until scrolled to.)
...
- Divi scrolls the WordPress page back to the top once it finishes loading whenever the address has a `#hash`, which is what made entry links jump to the entry and then bounce to the top. The snippet stops that at the source: it reads the `#hash`, hands it to the frame, then takes it off the WordPress page's address while the page loads, so Divi finds no hash and never scrolls. Just after the page's `load` it puts the hash back in the address bar (without scrolling) and re-applies the entry scroll once more, unless the visitor has already started scrolling (on the WordPress page, or inside the frame, which reports it as `{ frameUserScroll: true }`). The page itself also repeats its scroll request for a few seconds after load (`holdInitialScroll()`) as a second safeguard.
